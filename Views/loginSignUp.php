...
<style>
    #loginBgImage {
        background-image: url(<?php echo $_ENV['BASE_DIR'] . 'assets/images/quadrangle-blur.jpg' ?>);
        /* The image used */
        height: 100%;
...
<div id="loginBgImage" class="d-flex align-items-center justify-content-center" style="min-height: 85vh">
    <div>
        <!-- Alerts -->
        <?php include $absoluteDir . "views/components/alert.php"; ?>

        <?php if (isset($_REQUEST['userType']) && $_REQUEST['userType'] == 'newUser') { ?>
            <form method="POST" class="loginBack shadow rounded-3 p-3 w-30">
                ...
                <p class="text-black-50">
                    ...
                    <a href="<?php echo $_ENV['BASE_DIR'] . 'views/loginSignup.php' ?>" class="primaryColor text-decoration-none">Login Now</a>
                </p>
            </form>
        <?php } else { ?>
            <form method="POST" class="loginBack shadow rounded-3 p-3 w-30 my-auto">
                <h2 class="text-center darkColor my-3">Log In</h2>
                <input type="email" class="form-control mb-2" name="email" placeholder="Email" title="The mail id must contain sfc domain" />
                <input type="password" name="password" placeholder="Password" class="form-control mb-2" />
                <button type="submit" name="login" class="btn btn-outline-dark my-3 d-block">
                    Login
                </button>
                <p class="text-black-50">
                    Don't have an account yet?
                    <a href="<?php echo $_ENV['BASE_DIR'] . 'views/loginSignup.php?userType=newUser' ?>" class="primaryColor text-decoration-none">Signup Now</a>
                </p>
            </form>
        <?php } ?>
    </div>
